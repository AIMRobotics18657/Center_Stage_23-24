package org.firstinspires.ftc.teamcode.subsystems.settings;

public class GamepadSettings {

    // GAMEPAD 1
    public static final int EXPONENT_MODIFIER = 2;
    public static final double VX_WEIGHT = 1;
    public static final double VY_WEIGHT = 1;
    public static final double VRX_WEIGHT = .9;

    public static final double VX_WEIGHT_SLOW = 0.5;
    public static final double VY_WEIGHT_SLOW = 0.5;
    public static final double VRX_WEIGHT_SLOW = 0.45;

    public static final double GP1_STICK_DEADZONE = 0.05;
    public static final double GP1_TRIGGER_DEADZONE = 0.1;

    // GAMEPAD 2

    public static final double GP2_STICK_DEADZONE = 0.08;
    public static final double GP2_TRIGGER_DEADZONE = 0.1;

}
